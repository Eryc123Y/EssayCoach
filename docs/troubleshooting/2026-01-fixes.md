...
> Historical note: This incident log contains legacy `/api/v1/*` paths because it records January 2026 behavior. Current runtime proxy/API paths are `/api/v2/*`.
...
- [System Architecture - Database](../architecture/system-architecture.md) (If exists)
...
## Issue #9: Rubric Deletion Failure (RestrictedError)

**Date**: January 21, 2026
**Category**: Backend / Database
**Severity**: Medium (Maintenance blocked)

### Symptoms

- Attempting to delete a Rubric via API or Django Admin fails
- Error: `django.db.models.deletion.ProtectedError` (or similar `RestrictedError`)
- Root cause: Rubric is referenced by existing Tasks, which are referenced by Submissions

### Solution

**Switched to CASCADE Deletion**:
Modified `backend/core/models.py` to use `on_delete=models.CASCADE` for core relationships:
- `RubricItem` -> `MarkingRubric`
- `RubricLevelDesc` -> `RubricItem`
- `Task` -> `MarkingRubric`
- `Submission` -> `Task`
- `Feedback` -> `Submission`

This ensures that deleting a parent entity (like a Rubric) automatically cleans up all dependent records, preventing orphaned data and restricted errors during maintenance.

### Prevention

**Rule Added to AGENTS.md**:
> The database schema uses **CASCADE** deletion for core relationships. Always warn users before deleting entities that have dependent records.

---

## Issue #10: Frontend Scrolling Blocked by Layout Height
...
- Dashboard main content area does not scroll
- Content at the bottom of long pages (e.g., Rubric details) is cut off and inaccessible
- Sidebar remains fixed, but scroll events on the main area are ignored

### Root Cause

The flexbox layout in `src/app/dashboard/layout.tsx` lacked explicit height constraints and overflow rules for the scrollable container.

### Solution

Updated the main content container in `frontend/src/app/dashboard/layout.tsx`:
```tsx
<main className="flex-1 overflow-y-auto max-h-[calc(100vh-theme(spacing.16))]">
  <div className="p-6">
    {children}
  </div>
</main>
...
**Rule Added to AGENTS.md**:
> If dashboard content is cut off or scrolling is blocked, ensure the container has `max-h-[calc(100vh-theme(spacing.16))]` and `overflow-y-auto`.

---

## Issue #11: SyntaxError on 204 No Content Responses
...
- Console error: `SyntaxError: Unexpected end of JSON input`
- Occurs when performing successful DELETE or status updates
- Network tab shows `204 No Content` status, but the service layer fails

### Root Cause

The frontend service layer was calling `response.json()` on every response, but a `204 No Content` response has an empty body, which is not valid JSON.

### Solution

Updated `frontend/src/service/api/request.ts` (or individual service calls):
```typescript
...
**Rule Added to AGENTS.md**:
> Handle `204 No Content` responses safely by checking `response.status === 204` before calling `.json()`.

---

## Issue #12: Rubrics Menu Not Visible for Lecturers/Admins

**Date**: January 24, 2026
**Category**: Frontend / Authentication
**Severity**: Medium (Feature blocked for role-based users)

### Symptoms

- Users with lecturer or admin roles cannot see the "Rubrics" menu item in the sidebar
- The "Rubrics" menu should be visible per the navigation configuration
- All users appear to have student-level navigation regardless of actual role

### Root Cause

In `frontend/src/app/api/auth/login/route.ts`, the `user_role` cookie was being set from the original `user` object instead of the `normalizedUser` object. This caused:

```typescript
// Bug: Using wrong object for role
user_role: cookies().set('user_role', user.role)  // user.role is undefined

// Correct: Use normalizedUser
user_role: cookies().set('user_role', normalizedUser.role)
```

The backend returns user data with the field `role`, but the original `user` object (from response parsing) may not have this field properly mapped. The `normalizedUser` object correctly maps all fields including `role`.

### Solution

Updated login route to use `normalizedUser` for all cookie setters:

```typescript
// frontend/src/app/api/auth/login/route.ts
cookies().set('user_email', normalizedUser.email);
cookies().set('user_first_name', normalizedUser.first_name);
cookies().set('user_last_name', normalizedUser.last_name);
cookies().set('user_role', normalizedUser.role);  // FIXED: was user.role
cookies().set('user_id', String(normalizedUser.user_id || normalizedUser.id));
```

### Debug Logging Added

**Backend** (`backend/auth/views.py`):
```python
logger.info(f"[Login] Serialized user data: {json.dumps(user_data, ensure_ascii=False)}")
logger.info(f"[Login] Role field value: {user_data.get('role', 'NOT_FOUND')}")
```

**Frontend** (login route):
```typescript
console.log('[Login] Raw user object:', user);
console.log('[Login] Normalized user role:', normalizedUser.role);
```
...
1. Login as a lecturer or admin user
2. Open browser DevTools → Application → Cookies
3. Verify `user_role` cookie contains the correct role (e.g., `lecturer` or `admin`)
4. Refresh the dashboard
5. Check sidebar for "Rubrics" menu item visibility
6. Check console logs for correct role detection:
   ```
   [Sidebar Debug] User role: lecturer
   [Sidebar Debug] Rubrics: Roles=["lecturer","admin"], UserRole=lecturer, Visible=true
   ```

### Prevention

**Rule**: When setting user data in cookies during login, always use the `normalizedUser` object which has guaranteed field mappings. Never use the raw response object directly.

**Testing**: Add integration test to verify role-based navigation for each user type.

### Related Documentation

- [Role-Based Navigation](../architecture/system-architecture.md#role-based-navigation)
- [Authentication Architecture](../architecture/authentication.md)
- [Frontend Authentication Context](../frontend/README.md#authentication)
...
## Summary of Fixes Applied
 ...
 | #9 | Backend/DB | Restricted deletion error | ✅ Resolved |
 | #10 | Frontend/UI | Scrolling layout issue | ✅ Resolved |
 | #11 | Frontend/API | 204 JSON parse error | ✅ Resolved |
...
- [ ] `/api/v2/auth/me/` returns authenticated user data
...
- [ ] Deleting a Rubric successfully removes dependent Tasks and Submissions
- [ ] Dashboard content area scrolls properly on long pages
- [ ] DELETE requests return success without JSON parsing errors
