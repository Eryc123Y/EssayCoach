...
- `POST /api/v2/ai-feedback/agent/workflows/run/` – triggers the default workflow while sending the DSL inputs. The response includes `workflow_run_id`, `task_id`, `data`, and the input payload.
- `GET /api/v2/ai-feedback/agent/workflows/run/{workflow_run_id}/status/` – proxies Dify’s `/workflows/run/{workflow_run_id}` endpoint so the frontend can poll for `status`, `outputs`, and token usage.
