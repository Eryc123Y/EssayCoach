...
class RubricManager:
    ...
    def import_rubric_with_ai(
        ...
        try:
            ...
            final_rubric_name = rubric_name or parsed_data.get(
                "rubric_name", f"Rubric from {pdf_file.name}"
            )
            ...
            levels_count = sum(
                len(dim.get("levels", [])) for dim in parsed_data.get("dimensions", [])
            )

            logger.info(
                f"Successfully imported rubric {rubric.rubric_id} "
                f"with {items_count} items and {levels_count} levels"
            )
    ...
    def _detect_if_rubric(self, parsed_data: dict[str, Any]) -> tuple[bool, str]:
        ...
        if not is_rubric:
            reason = parsed_data.get(
                "reason", "Document does not appear to be a rubric"
            )
            return False, reason
    ...
    def _validate_rubric_data(self, parsed_data: dict[str, Any]) -> None:
        ...
        for idx, dimension in enumerate(dimensions):
            ...
            if weight is None:
                raise RubricImportError(
                    f"Dimension '{dimension.get('name')}' missing weight"
                )
            ...
            try:
                ...
                if weight_decimal <= 0:
                    raise RubricImportError(
                        f"Dimension '{dimension.get('name')}' has non-positive weight: {weight}"
                    )
                total_weight += weight_decimal
            except (ValueError, TypeError) as e:
                raise RubricImportError(
                    f"Invalid weight for dimension '{dimension.get('name')}': {weight}"
                ) from e

            levels = dimension.get("levels", [])
            if not levels:
                raise RubricImportError(
                    f"Dimension '{dimension.get('name')}' has no levels"
                )

            for level_idx, level in enumerate(levels):
                if not level.get("name"):
                    raise RubricImportError(
                        f"Level {level_idx} in dimension '{dimension.get('name')}' missing name"
                    )
        ...
        if not (Decimal("99") <= total_weight <= Decimal("101")):
            raise RubricImportError(
                f"Rubric weights must sum to ~100, got {total_weight} "
                "(allowed range: 99-101 for rounding tolerance)"
            )

        logger.info(
            f"Rubric validation passed: {len(dimensions)} dimensions, total weight={total_weight}"
        )

    @transaction.atomic
    def _create_rubric_in_db(
        self, parsed_data: dict[str, Any], user: User, rubric_name: str
    ) -> MarkingRubric:
        ...
        try:
            rubric = MarkingRubric.objects.create(
                user_id_user=user, rubric_desc=rubric_name
            )
            ...
            for dimension in parsed_data["dimensions"]:
                ...
                logger.debug(
                    f"Created RubricItem {rubric_item.rubric_item_id}: {dimension['name']}"
                )
