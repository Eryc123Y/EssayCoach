...
class UserRegistrationSerializer(serializers.Serializer):
    ...
    first_name = serializers.CharField(max_length=20, required=False, allow_blank=True, allow_null=True)
    last_name = serializers.CharField(max_length=20, required=False, allow_blank=True, allow_null=True)
    role = serializers.CharField(max_length=10, required=False, allow_blank=True, allow_null=True)
    ...
    def create(self, validated_data: dict[str, Any]) -> User:
        ...
        # Create user
        user = User.objects.create_user(user_email=email, password=password, **validated_data)
        return user
...
@extend_schema_serializer(
    examples=[
        OpenApiExample(
            "Login Request",
            value={"email": "student@example.com", "password": "SecurePassword123!"},
        )
    ]
)
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    - name: Full name (combined first and last name)
    ...
    username = serializers.EmailField(source="user_email", read_only=True)
    ...
    def get_name(self, obj: User) -> str:
        ...
        return str(f"{obj.user_fname or ''} {obj.user_lname or ''}".strip() or obj.user_email)

    def get_avatar(self, obj: User) -> str | None:
        """Return placeholder for avatar."""
        return None


@extend_schema_serializer(
    examples=[OpenApiExample("User Update Request", value={"first_name": "Jane", "last_name": "Smith"})]
)
