...
class UserRegistrationTests(AuthAPITestCase):
    ...
    def test_registration_invalid_email(self):
        ...
        data = {"email": "invalid-email", "password": "SecurePassword123!", "password_confirm": "SecurePassword123!"}
        response = self.client.post(f"{self.base_url}register/", data, format="json")
...
class UserLoginTests(AuthAPITestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.user = self._create_test_user(email="login@example.com", password="LoginPassword123!")

    def test_login_success(self):
        """Test successful login."""
        data = {"email": "login@example.com", "password": "LoginPassword123!"}
...
class UserLogoutTests(AuthAPITestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.user = self._create_test_user(email="logout@example.com", password="Password123!")
...
class PasswordResetTests(AuthAPITestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.user = self._create_test_user(email="reset@example.com", password="OldPassword123!")
    ...
    def test_password_reset_invalid_email(self):
        ...
        data = {"email": "invalid-email", "new_password": "NewPassword123!", "new_password_confirm": "NewPassword123!"}
        response = self.client.post(f"{self.base_url}password-reset/", data, format="json")
...
class PasswordChangeTests(AuthAPITestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.user = self._create_test_user(email="change@example.com", password="OldPassword123!")
        self.token, _ = Token.objects.get_or_create(user=self.user)
        self.client.credentials(HTTP_AUTHORIZATION=f"Token {self.token.key}")
...
class CurrentUserTests(AuthAPITestCase):
    ...
    def setUp(self):
        ...
        self.user = self._create_test_user(
            email="me@example.com", password="Password123!", first_name="Original", last_name="Name", role="student"
        )
        self.token, _ = Token.objects.get_or_create(user=self.user)
        self.client.credentials(HTTP_AUTHORIZATION=f"Token {self.token.key}")
...
class TokenManagementTests(AuthAPITestCase):
    ...
    def test_token_created_on_registration(self):
        ...
        data = {"email": "token@example.com", "password": "Password123!", "password_confirm": "Password123!"}
        response = self.client.post(f"{self.base_url}register/", data, format="json")
    ...
    def test_token_created_on_login(self):
        ...
        user = self._create_test_user(email="logintoken@example.com", password="Password123!")

        data = {"email": "logintoken@example.com", "password": "Password123!"}
        response = self.client.post(f"{self.base_url}login/", data, format="json")
    ...
    def test_token_reused_on_multiple_logins(self):
        ...
        user = self._create_test_user(email="reuse@example.com", password="Password123!")

        # First login
        data = {"email": "reuse@example.com", "password": "Password123!"}
    ...
    def test_token_deleted_on_logout(self):
        ...
        user = self._create_test_user(email="logouttoken@example.com", password="Password123!")
