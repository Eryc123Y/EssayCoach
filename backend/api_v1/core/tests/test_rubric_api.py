...
def api_client():
    ...
    client = APIClient()
    user = User.objects.create_user(user_email="lecturer@example.com", password="password123", user_role="lecturer")
...
def sample_rubric(db, api_client):
    ...
    _, user = api_client
    rubric = MarkingRubric.objects.create(user_id_user=user, rubric_desc="Existing Rubric")
    item = RubricItem.objects.create(
        rubric_id_marking_rubric=rubric,
        rubric_item_name="Content",
        rubric_item_weight=50.0,
    )
...
class TestRubricAPI:
    """Test suite for Rubric API endpoints."""

    @patch("api_v1.core.rubric_manager.RubricManager")
    def test_import_rubric_pdf_success(self, mock_manager_class, api_client):
        ...
        mock_manager.import_rubric_with_ai.return_value = expected_result

        url = reverse("rubrics-import-from-pdf-with-ai")
        pdf_file = SimpleUploadedFile("rubric.pdf", b"pdf content", content_type="application/pdf")

        with patch("django.conf.settings.SILICONFLOW_API_KEY", "fake-api-key"):
    ...
    def test_import_rubric_pdf_name_override(self, mock_manager_class, api_client):
        ...
        assert args[2] == "My Custom Rubric"
        assert args[1] == user
    ...
    def test_import_rubric_pdf_not_a_rubric(self, mock_manager_class, api_client):
        ...
        url = reverse("rubrics-import-from-pdf-with-ai")
        pdf_file = SimpleUploadedFile("essay.pdf", b"essay content", content_type="application/pdf")
    ...
    def test_import_rubric_pdf_invalid_type(self, api_client):
        ...
        txt_file = SimpleUploadedFile("rubric.txt", b"text content", content_type="text/plain")
    ...
    def test_get_rubric_detail(self, api_client, sample_rubric):
        ...
        client, _ = api_client
        url = reverse("rubrics-detail-with-items", kwargs={"pk": sample_rubric.rubric_id})
    ...
    def test_get_rubric_detail_not_found(self, api_client):
        ...
        client, _ = api_client
        url = reverse("rubrics-detail-with-items", kwargs={"pk": 9999})
