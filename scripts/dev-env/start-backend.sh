...
if python -c "import django; print(f'Django {django.get_version()} found')" 2>/dev/null; then
    ...
    if ! python manage.py migrate --noinput 2>&1; then
        echo "[ERROR] Django migrations failed. Please check the error above."
        exit 1
    fi
    echo "[SUCCESS] Django migrations completed successfully."
...
# Target credentials
email_candidates = ['admin@example.com', 'admin']
password = 'admin'
...
with connection.cursor() as cur:
    ...
    for email in email_candidates:
        cur.execute(
            ...
            '    is_superuser = TRUE, is_staff = TRUE, is_active = TRUE, user_email = %s\n'
            'WHERE user_email = %s\n'
            'RETURNING user_id',
            [hashed, 'admin', 'active', 'admin@example.com', email]
        )
        if cur.rowcount:
            updated = True
            break
    ...
    if not updated:
        # Insert a new admin user with the requested username/email 'admin@example.com'
        ...
        cur.execute(
            ...
            [next_id, 'Admin', 'User', 'admin@example.com', 'admin', 'active', hashed]
        )

print('Admin user ensured (email: "admin@example.com")')
PY
