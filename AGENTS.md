...
This file is the primary instruction set for AI coding agents (Cursor, Claude Code, Codex, etc.) in this repository.

> **IMPORTANT**: **AGENTS.md** (this file) is the single source of truth for project status and agent instructions. Update it after every significant code change to reflect the current state and next priorities.
>
...
## Quick navigation (JIT index)

EssayCoach is a Django + Next.js monorepo for AI-assisted essay feedback, rubric management, and classroom workflows. Makefile-first, API-v2-first; PRDs in `docs/prd/` are the feature source of truth.

### Repository layout
```text
EssayCoach/
├── backend/      # Django app, models, API v2, AI integration
├── frontend/     # Next.js app router UI + API proxy
├── docs/         # PRDs, architecture, learnings, generated docs
├── scripts/      # dev health/recovery + docs generation
└── AGENTS.md     # authoritative project status + agent instructions (this file)
```

### Deeper index files
- Backend: `backend/AGENTS.md`
- Backend core domain: `backend/core/AGENTS.md`
- Backend API layer: `backend/api_v2/AGENTS.md`
- Backend AI integration: `backend/ai_feedback/AGENTS.md`
- Frontend: `frontend/AGENTS.md`
- Frontend app/router layer: `frontend/src/app/AGENTS.md`
- Frontend feature slices: `frontend/src/features/AGENTS.md`
- Frontend API service layer: `frontend/src/service/api/v2/AGENTS.md`
- Docs: `docs/AGENTS.md`
- Docs architecture references: `docs/architecture/AGENTS.md`
- Docs implementation memory: `docs/learnings/AGENTS.md`
- PRDs: `docs/prd/AGENTS.md`

### Where to look
| Task | Location | Notes |
|---|---|---|
| Start dev/test/build | `Makefile` | Primary command surface |
| Backend route assembly | `backend/api_v2/api.py` | mounts all v2 routers |
| Core data model | `backend/core/models.py` | central ORM model file |
| Backend business logic | `backend/core/services.py` | dashboard/task/rubric services |
| Backend core guidance | `backend/core/AGENTS.md` | centralized models + seed/service rules |
| AI workflow endpoints | `backend/api_v2/ai_feedback/views.py` | Dify + optional LangGraph (`get_essay_agent`) |
| Frontend route shell | `frontend/src/app/` | App Router pages/layouts/API handlers |
| Frontend route guidance | `frontend/src/app/AGENTS.md` | proxy/auth/parallel-route rules |
| Frontend auth/JWT rules | `frontend/src/lib/auth.ts` | `jose` verification + CSRF helpers |
| Frontend proxy boundary | `frontend/src/app/api/v2/[...path]/route.ts` | safe-header allowlist + cookie auth |
| Frontend feature code | `frontend/src/features/` | feature-slice modules |
| Frontend API contracts | `frontend/src/service/api/v2/types.ts` | large TS contract registry |
| Architecture references | `docs/architecture/AGENTS.md` | useful, but verify against current code |
| Feature requirements | `docs/prd/` | numbered PRDs + design assets |
| Historical decisions | `docs/learnings/` | implementation logs and lessons |

### Code map
| Symbol | Type | Location | Role |
|---|---|---|---|
| `api_v2` | NinjaAPI | `backend/api_v2/api.py` | backend v2 root |
| `DashboardService` | class | `backend/core/services.py` | dashboard aggregation |
| `validateAndDecodeToken` | function | `frontend/src/lib/auth.ts` | server-side JWT verification |
| `proxy` | function | `frontend/src/app/api/v2/[...path]/route.ts` | frontend→backend bridge |
| `useDashboardData` | hook | `frontend/src/features/dashboard/hooks/useDashboardData.ts` | role dashboard fetch/retry |

### Repo conventions (summary)
- New API work targets `api/v2` only.
- Backend uses `uv`; frontend uses `pnpm`.
- Root `.env`; frontend-specific env in `frontend/.env.local`.
- Backend constrained strings live in `backend/api_v2/types/enums.py` as `StrEnum`.
- Frontend auth is hybrid: httpOnly cookies for tokens, localStorage/context for user data.
- Keep **this file** current after significant code changes.

### Anti-patterns (summary)
- Never parse JWTs with `atob()`; use `jose` verification.
- Never move refresh tokens into localStorage or client request bodies.
- Do not remove DRF blindly; `simplejwt` depends on it here.
- Do not use Next rewrites instead of the route-handler proxy for auth forwarding.
- Do not forget `credentials: 'include'` on cookie-authenticated frontend requests.
- Do not trust `localhost` defaults in dev; use `127.0.0.1`.
- New files under `frontend/src/features/tasks/` may require `git add -f` because of root `tasks/` gitignore.

### One-liner commands
```bash
make install && make dev    # full stack after `make db`
make health                   # recover stale dev state
make test && make lint && make typecheck && make build
```
...
## Agent Team Workflow Learnings (2026-02-25)

During Dashboard Phase 2 implementation, key learnings included:
- Parallel routes require all slots.
- Icon imports: use `@tabler/icons-react` directly.
- Design compliance: check typography scale.
- Security first: JWT signature verification, CSRF tokens required.
...
| Issue | Description |
|-------|-------------|
| Single source of truth | AGENTS.md updated with accurate status after security audit |
