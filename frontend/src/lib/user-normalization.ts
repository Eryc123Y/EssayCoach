export function normalizeUserInfo(raw: any): any {
  ...
  const normalizedRole =
    roleRaw === 'teacher'
      ? 'lecturer'
      : (['student', 'lecturer', 'admin'].includes(roleRaw) ? roleRaw : 'student');

  const normalizedUserId = Number(raw.user_id ?? raw.id ?? 0);
  const normalizedEmail = String(raw.user_email ?? raw.email ?? '');
  const normalizedFirstName = (raw.user_fname ?? raw.first_name ?? null) as string | null;
  const normalizedLastName = (raw.user_lname ?? raw.last_name ?? null) as string | null;
  const normalizedIsActive =
    typeof raw.is_active === 'boolean'
      ? raw.is_active
      : (raw.status ? raw.status !== 'suspended' : true);
