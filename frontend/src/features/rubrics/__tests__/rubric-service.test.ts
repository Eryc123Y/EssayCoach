...
describe('rubricActionsService', () => {
  ...
  describe('duplicateRubric', () => {
    ...
    it('duplicates a rubric as public', async () => {
      ...
      const result = await rubricActionsService.duplicateRubric(5, {
        visibility: 'public'
      });
    ...
    it('throws on 403 when duplicating a private rubric you do not own', async () => {
      (global.fetch as any).mockResolvedValueOnce({
        ...
        status: 403,
        json: async () => ({
          message: "Cannot duplicate a private rubric you don't own"
        })
      });
      await expect(
        rubricActionsService.duplicateRubric(7, {})
      ).rejects.toThrow();
    });
    ...
    it('throws on 404 when rubric does not exist', async () => {
      ...
      await expect(
        rubricActionsService.duplicateRubric(999, {})
      ).rejects.toThrow();
    });
  });
});
