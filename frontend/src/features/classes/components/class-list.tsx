...
export function ClassList() {
  ...
  const activeClasses = filteredClasses.filter((c) => c.class_status === 'active');
  const archivedClasses = filteredClasses.filter((c) => c.class_status === 'archived');
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="active">
        <TabsList>
          ...
          <TabsTrigger value="archived">Archived ({archivedClasses.length})</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="active" className="mt-4">
          {activeClasses.length === 0 ? (
            <div className="text-center py-12">
              ...
              {canCreateClass && (
                <Button variant="link" onClick={() => router.push('/dashboard/classes/new')} className="mt-2">
                  Create your first class
                </Button>
              )}
              {canJoinClass && (
                <Button variant="link" onClick={() => setShowJoinDialog(true)} className="mt-2">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {activeClasses.map((cls) => (
                <ClassCard key={cls.class_id} classItem={cls} onUpdate={loadClasses} />
              ))}
            </div>
          )}
        </TabsContent>
