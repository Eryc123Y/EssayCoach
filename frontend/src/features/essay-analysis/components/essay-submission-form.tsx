...
import { Sparkles, ArrowRight, ArrowLeft, Loader2, BookOpen, FileText, ClipboardCheck, CheckCircle2, Lightbulb, Target, TrendingUp } from 'lucide-react';
...
const steps = [
  { id: 1, title: 'Question', icon: BookOpen, description: 'Enter your essay topic' },
  { id: 2, title: 'Content', icon: FileText, description: 'Write or paste your essay' },
  { id: 3, title: 'Review', icon: ClipboardCheck, description: 'Review and submit' },
];

const editorNotes = [
  { icon: Lightbulb, title: 'Lead with clarity', desc: 'State your purpose early and keep it focused.' },
  { icon: Target, title: 'Claim + evidence', desc: 'Support each point with concrete examples.' },
  { icon: TrendingUp, title: 'Revise for flow', desc: 'Read aloud to catch awkward transitions.' },
];
...
export function EssaySubmissionForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (question && content) {
      ...
      onSubmit({ question, content, rubricId: rubricId ? parseInt(rubricId) : undefined });
    }
  };
                ...
                return (
                  <div key={step.id} className='relative flex gap-3'>
                    {index < steps.length - 1 && (
                      <span className={cn(
                        'absolute left-4 top-9 h-[calc(100%-10px)] w-px',
                        isCompleted ? 'bg-emerald-500 dark:bg-emerald-600' : 'bg-slate-200 dark:bg-slate-700'
                      )} />
                    )}
                    <div className={cn(
                      'flex h-8 w-8 items-center justify-center rounded-full border text-xs font-semibold transition-all duration-300',
                      isActive && 'border-emerald-500 bg-emerald-500 text-white shadow-md dark:shadow-emerald-950',
                  ...
                  {currentStep === 1 && (
                    ...
                    >
                      ...
                      <p className='text-sm text-slate-500 dark:text-slate-400'>
                        Enter the prompt or topic you want reviewed. The clearer the focus, the sharper the feedback.
                      </p>
                    </motion.div>
                  )}
                    ...
                    >
                      ...
                      <div className='space-y-3'>
                        <Label className='text-base font-semibold text-slate-700 dark:text-slate-200'>
                          Grading Rubric 
                          <span className='text-slate-400 dark:text-slate-500 font-normal ml-2'>(Optional)</span>
                        </Label>
                        <Select value={rubricId} onValueChange={setRubricId} disabled={isSubmitting}>
                          ...
                          <SelectContent>
                            ...
                            {rubrics.map((r) => (
                              <SelectItem key={r.rubric_id} value={r.rubric_id.toString()}>
                                {r.rubric_desc}
                              </SelectItem>
                            ))}
                          </SelectContent>
                  ...
                  {currentStep === 3 && (
                    ...
                    >
                      ...
                      <div className={cn(
                        ...
                        <div className='grid grid-cols-3 gap-4 text-sm'>
                          <div>
                            ...
                            <span className='font-semibold text-slate-800 dark:text-slate-100'>{wordCount}</span>
                          </div>
                          <div>
                            <span className='text-slate-500 dark:text-slate-400'>Characters:</span>{' '}
                            <span className='font-semibold text-slate-800 dark:text-slate-100'>{charCount}</span>
                          </div>
                          <div>
                            <span className='text-slate-500 dark:text-slate-400'>Rubric:</span>{' '}
                      ...
                      <div className={cn(
                        ...
                        <h4 className='font-semibold mb-3 text-slate-800 dark:text-slate-100'>Question</h4>
                        <p className={cn(
                          'text-slate-600 dark:text-slate-300',
                          'bg-slate-50 dark:bg-slate-950',
                          'p-3 rounded-md border border-slate-200 dark:border-slate-700'
                        )}>
                      ...
                      <div className={cn(
                        ...
                        <div className='flex justify-between mb-3'>
                          <h4 className='font-semibold text-slate-800 dark:text-slate-100'>Content Preview</h4>
                          <span className='text-sm text-slate-500 dark:text-slate-400'>{wordCount} words</span>
                        </div>
                        <p className={cn(
                          'text-slate-600 dark:text-slate-300',
                          'bg-slate-50 dark:bg-slate-950',
