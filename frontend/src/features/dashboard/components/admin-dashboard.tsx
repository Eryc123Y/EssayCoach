    ...
    <div className='space-y-6'>
      ...
      <section>
        ...
        <SystemHealth
          ...
          health={data.stats.systemHealth}
        />
      </section>
  ...
  return (
    <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
        ...
        <CardContent className='space-y-4'>
          <div className='flex items-center justify-between'>
            ...
            <Badge
              ...
              className={healthConfig.className}
            >
              {healthConfig.label}
            </Badge>
          </div>
      ...
      <Card className='bg-card border-slate-200 shadow-sm dark:border-slate-800'>
        ...
        <CardContent className='space-y-4'>
          <ActivityItem
            icon={<IconDatabase className='text-muted-foreground h-4 w-4' />}
            label='Submissions'
...
function UserMetrics({ data }: { data: AdminDashboardResponse }) {
  ...
  const studentPercent =
    total > 0 ? Math.round((data.stats.activeStudents / total) * 100) : 0;
  const lecturerPercent =
    total > 0 ? Math.round((data.stats.activeLecturers / total) * 100) : 0;

  const activeStudentsDisplay =
    data.stats.activeStudents === 0
      ? '0'
      : data.stats.activeStudents.toLocaleString();
  const activeLecturersDisplay =
    data.stats.activeLecturers === 0
      ? '0 lecturers'
      ...
      <CardContent className='space-y-4'>
        <div className='grid grid-cols-2 gap-4'>
          ...
          <div className='space-y-2'>
            ...
            <p className='text-muted-foreground text-xs'>Active Lecturers</p>
          </div>
        </div>
      ...
      <section>
        ...
        <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'>
          {[1, 2, 3, 4].map((i) => (
            <Card
              key={i}
              className='h-32 animate-pulse rounded bg-slate-100 dark:bg-slate-800'
            />
          ))}
        </div>
      </section>
