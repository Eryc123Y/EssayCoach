...
function CustomTooltip({ active, payload, label }: CustomTooltipProps) {
  if (active && payload && payload.length) {
    const essayCount = payload.find((p) => p.dataKey === 'essay_count')?.value || 0;
    const avgScore = payload.find((p) => p.dataKey === 'average_score')?.value || 0;
...
export function ProfileProgressTab({ progress }: ProfileProgressTabProps) {
  const data = progress?.entries && progress.entries.length > 0
    ? progress.entries
    : MOCK_PROGRESS_DATA;
  ...
  const avgScore =
    data.length > 0
      ? Math.round(
          data.filter((e) => e.average_score !== null).reduce((sum, e) => sum + (e.average_score || 0), 0) /
  ...
  const firstHalfAvg =
    firstHalf.filter((e) => e.average_score !== null).length > 0
      ? firstHalf.filter((e) => e.average_score !== null).reduce((sum, e) => sum + (e.average_score || 0), 0) /
        firstHalf.filter((e) => e.average_score !== null).length
      : 0;

  const secondHalfAvg =
    secondHalf.filter((e) => e.average_score !== null).length > 0
      ? secondHalf.filter((e) => e.average_score !== null).reduce((sum, e) => sum + (e.average_score || 0), 0) /
    ...
    <div className="space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 dark:text-white">{totalEssays}</p>
              </div>
      ...
      <Card className="border-slate-200 dark:border-slate-700">
        <CardHeader>
          ...
          <p className="text-sm text-slate-500">Monthly essay submissions and average scores</p>
        </CardHeader>
        <CardContent>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <YAxis
                  ...
                  className="text-xs text-slate-500"
                  tick={{ fill: '#64748b', fontSize: 12 }}
                  label={{ value: 'Essays', angle: -90, position: 'insideLeft', fill: '#64748b', fontSize: 12 }}
                />
                <YAxis
                  yAxisId="right"
                  orientation="right"
