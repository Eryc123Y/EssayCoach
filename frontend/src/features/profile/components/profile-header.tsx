...
function getRoleInfo(role: string): { label: string; color: string } {
  switch (role) {
    ...
    case 'lecturer':
      return { label: 'Lecturer', color: 'bg-indigo-100 text-indigo-700 border-indigo-200' };
    case 'student':
      return { label: 'Student', color: 'bg-emerald-100 text-emerald-700 border-emerald-200' };
    default:
 ...
 */
export function ProfileHeader({ user, avatarUrl, joinedDate }: ProfileHeaderProps) {
  ...
  return (
    <Card className="border-slate-200 bg-gradient-to-br from-slate-50 to-white shadow-sm dark:from-slate-900 dark:to-slate-800 dark:border-slate-700">
